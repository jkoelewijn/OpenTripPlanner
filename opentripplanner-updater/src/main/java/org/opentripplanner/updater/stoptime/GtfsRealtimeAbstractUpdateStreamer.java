...
public abstract class GtfsRealtimeAbstractUpdateStreamer implements UpdateStreamer {
    ...
    protected TripUpdate getUpdateForAddedTrip(AgencyAndId tripId,
        ...
        if(trip.getRoute() == null){
            LOG.warn("Created added trip without route for " + routeId);
        }
    ...
    protected TripUpdate getUpdateForScheduledTrip(AgencyAndId tripId,
        ...
        if(tripUpdate.hasVehicle()) {
            GtfsRealtime.VehicleDescriptor vehicle = tripUpdate.getVehicle();
            if(vehicle.hasExtension(GtfsRealtime.wheelchairAccessible)) {
                return TripUpdate.forUpdatedTrip(tripId, timestamp, serviceDate,
                        updates, vehicle.getExtension(GtfsRealtime.wheelchairAccessible));
            }
        }
